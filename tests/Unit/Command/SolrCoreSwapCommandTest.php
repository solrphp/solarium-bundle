 ...
 */
class SolrCoreSwapCommandTest extends TestCase
{
    /**
    ...
    public function testExecuteNoCoreArgument(): void
    {
        ...
        $application->add(new SolrCoreSwapCommand($manager));

        $command = $application->find('solr:core:swap');
    ...
    public function testExecute(): void
    {
        ...
        $manager = $this->getMockBuilder(CoreManager::class)->disableOriginalConstructor()->getMock();
        $manager->expects(self::once())->method('swap')->willReturn($response);
        ...
        self::assertSame('successfully swapped cores foo & bar', trim($commandTester->getDisplay()));
    }
    ...
    public function testExecutionFail(?Error $error, string $message): void
    {
        ...
        self::assertSame('error swapping cores foo & bar: '.$message.' (1)', trim($commandTester->getDisplay()));
    }
